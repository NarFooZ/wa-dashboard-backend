...
router.post('/rules', requireAuth, (req, res) => {
  const { keyword, emoji, amount, description, order_type } = req.body;
  if (!keyword || !amount) return res.status(400).json({ error: 'الكلمة والمبلغ مطلوبان' });
  const validType = ['internal', 'external', 'subscription'].includes(order_type) ? order_type : 'internal';

  const result = db.prepare(`
    INSERT INTO billing_rules (keyword, emoji, amount, description, order_type)
    VALUES (?, ?, ?, ?, ?)
  `).run(keyword, emoji || null, parseFloat(amount), description || '', validType);
...
router.patch('/rules/:id', requireAuth, (req, res) => {
  const { keyword, emoji, amount, description, is_active, order_type } = req.body;
  const validType = order_type && ['internal', 'external', 'subscription'].includes(order_type) ? order_type : null;
  db.prepare(`
    ...
    WHERE id = ?
  `).run(keyword, emoji, amount ? parseFloat(amount) : null, description, is_active, validType, req.params.id);
...
router.get('/transactions', requireAuth, (req, res) => {
  ...
  const allowedTypes = ['credit', 'debit', 'manual', 'deposit', 'settlement'];
  const typeFilter = allowedTypes.includes(type) ? type : null;
  ...
  const transactions = db.prepare(`
    ...
    LIMIT ? OFFSET ?
  `).all(typeFilter, typeFilter, parseInt(limit), parseInt(offset));
